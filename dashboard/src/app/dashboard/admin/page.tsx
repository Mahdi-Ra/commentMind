...
function UsersTable({ users, savingId, onUpdate }: { users: AdminUser[]; savingId: string | null; onUpdate: (id: string, data: { plan?: string; is_active?: boolean }) => void }) {
  ...
  return <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white"><table className="w-full min-w-[780px] text-left text-sm"><thead className="bg-slate-50 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">User</th><th className="px-4 py-3">Plan</th><th className="px-4 py-3">Sites</th><th className="px-4 py-3">Comments</th><th className="px-4 py-3">Status</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody className="divide-y divide-slate-100">{users.map((item) => <tr key={item.id}><td className="px-4 py-3"><p className="font-medium text-slate-900">{item.full_name || 'Unnamed user'}</p><p className="text-xs text-slate-500">{item.email}</p></td><td className="px-4 py-3"><select value={item.plan} disabled={savingId === item.id} onChange={(e) => onUpdate(item.id, { plan: e.target.value })} className="rounded-md border border-slate-200 bg-white px-2 py-1 text-sm capitalize outline-none focus:border-violet-400">{['free', 'starter', 'pro', 'agency'].map((plan) => <option key={plan}>{plan}</option>)}</select></td><td className="px-4 py-3 tabular-nums">{item.sites_count}</td><td className="px-4 py-3 tabular-nums">{item.comments_count.toLocaleString()}</td><td className="px-4 py-3"><Badge variant={item.is_active ? 'success' : 'danger'}>{item.is_active ? 'Active' : 'Suspended'}</Badge></td><td className="px-4 py-3 text-right"><Button size="sm" variant={item.is_active ? 'outline' : 'primary'} loading={savingId === item.id} onClick={() => onUpdate(item.id, { is_active: !item.is_active })}>{item.is_active ? 'Suspend' : 'Activate'}</Button></td></tr>)}</tbody></table></div>
}
